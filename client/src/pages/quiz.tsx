...
export default function Quiz() {
  ...
  const saveAnswerMutation = useMutation({
    mutationFn: async (data: { questionId: number; answer: string }) => {
      return await apiRequest(
        "POST",
        `/api/user/quizzes/${quizType}/answers`,
        data
      );
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      toast({
        ...
        description:
          "There was a problem saving your answer. Please try again.",
        variant: "destructive",
      });
  ...
  const completeQuizMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest(
        "POST",
        `/api/user/quizzes/${quizType}/complete`,
        {}
      );
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      toast({
        ...
        description:
          "There was a problem completing your quiz. Please try again.",
        variant: "destructive",
      });
          ...
          <CardContent className="p-8">
            ...
            >
              {currentQuestion.options.map((option) => (
                <div
                  key={option.id}
                  className="flex items-center space-x-2 quiz-option p-3 rounded-lg border border-gray-200 hover:border-blue-200 cursor-pointer"
                >
                  <RadioGroupItem
                    id={option.id}
