...
export default function AiCounselor() {
  ...
  const { showChatInterface, toggleChatInterface, currentUser } = useAppContext();
  ...
  if (!showChatInterface) {
    return (
      <button 
        ...
        className="fixed right-6 bottom-20 md:bottom-6 bg-primary-500 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg z-20"
      >
          ...
          <div className="flex justify-between items-center px-4 py-3 border-b">
            <div className="flex items-center">
              <div className="w-8 h-8 rounded-full bg-primary-100 flex items-center justify-center mr-3">
                <span className="material-icons text-primary-500 text-sm">smart_toy</span>
              </div>
              <div>
                <h3 className="font-poppins font-semibold text-md">{t('ai_counselor_name')}</h3>
                <div className="text-xs text-gray-500">{t('ai_status')}</div>
              </div>
            </div>
            <button className="text-gray-400 hover:text-gray-600" onClick={toggleChatInterface}>
              <span className="material-icons">close</span>
            </button>
          </div>
            ...
            <div className="space-y-4">
              {messages.map((msg) => (
                ...
                >
                  ...
                  <div className={`py-2 px-4 max-w-[80%] ${
                    msg.role === 'assistant' 
                      ? 'bg-gray-100 rounded-lg rounded-tl-none text-gray-800' 
                      : 'bg-primary-100 text-primary-800 rounded-lg rounded-tr-none'
                  }`}>
                    <p className="text-sm whitespace-pre-line">{msg.content}</p>
                  </div>
                  
                  {msg.role === 'user' && (
                    <div className="w-8 h-8 rounded-full bg-primary-500 flex items-center justify-center ml-3 flex-shrink-0">
          ...
          <div className="p-3 border-t">
            <div className="flex items-center">
              ...
              <Button 
                ...
                className="ml-2 bg-primary-500 hover:bg-primary-600 text-white rounded-full w-10 h-10 p-0 flex items-center justify-center"
