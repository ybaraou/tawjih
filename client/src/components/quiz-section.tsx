  ...
  return (
    <div className="mb-8">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-poppins font-semibold text-xl">{t('career_assessment_quiz')}</h3>
        <a href="#" className="text-primary-500 text-sm font-medium">{t('view_all')}</a>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sortedQuizzes.slice(0, 2).map((quiz) => (
          <Card key={quiz.id} className={`overflow-hidden ${quiz.completed ? 'border-green-100' : ''}`}>
            <CardContent className="p-5">
              <div className="flex justify-between items-center mb-3">
                <Badge variant={quiz.completed ? "success" : "warning"} className={quiz.completed ? "bg-green-100 text-secondary-600" : "bg-yellow-100 text-accent-500"}>
              ...
              <h4 className="font-poppins font-semibold text-lg mb-2">
                {quiz.quizId === 1 ? t('personality_assessment') : t('skills_interests')}
              </h4>
              
              <p className="text-gray-600 text-sm mb-4">
                {quiz.quizId === 1 ? t('personality_description') : t('skills_description')}
              </p>
                ...
                <>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-500">
                      {Math.round(quiz.progress / 100 * 15)}/15 {t('questions_completed')}
                    </span>
                    <Button 
                      size="sm"
              ...
              {quiz.completed && (
                <div className="flex justify-between items-center">
                  ...
                  <Button 
                    ...
                    className="text-primary-500 font-medium p-0 h-auto"
                  >
                    {t('view_results')}
                  </Button>
                </div>
